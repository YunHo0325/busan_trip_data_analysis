		...
		<section>
			<div id="main">
				...
				<table id="user_table" class="table">
					...
					%>
						<td id="idx">
							<a href="noticeDetail.jsp?num=<%=notice.getnNum() %>&uId=<%=member.getuId() %>"><%=notice.getnNum() %></a>
						</td>
					...
					<%}else{ %>
						<td id="idx">
							<a href="noticeDetail.jsp?num=<%=notice.getnNum() %>"><%=notice.getnNum() %></a>
						</td>
						<td id="element_subject">
...
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=startPage - pageBlock %>&uId=<%=uId %>&filter=<%=filter %>&search_text=<%=search_text %>">[이전]</a>
<%			
		}
		
		for(int i=startPage; i <= endPage; i++){
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=i%>&filter=<%=filter %>&uId=<%=uId %>&search_text=<%=search_text %>">
								[<%=i %>]
							</a>			
<%
		}
		...
		if(endPage < pageCount){
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=startPage + pageBlock %>&uId=<%=uId %>&filter=<%=filter %>&search_text=<%=search_text %>">[다음]</a>
<%		
		}
...
<%
	if (count > 0){
		...
		if(startPage > pageBlock){
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=startPage - pageBlock %>&filter=<%=filter %>&search_text=<%=search_text %>">[이전]</a>
<%			
		}
		
		for(int i=startPage; i <= endPage; i++){
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=i%>&filter=<%=filter %>&search_text=<%=search_text %>">
								[<%=i %>]
							</a>			
<%
		}
		...
		if(endPage < pageCount){
%>
							<a href = "noticeSearchProc.jsp?pageNum=<%=startPage + pageBlock %>&filter=<%=filter %>&search_text=<%=search_text %>">[다음]</a>
<%		
		}
